param(
    ...
    [string]$Abi = "cp312",
    [string]$Out = "results/Table1_paper_standard.json",
    [switch]$SkipDownload,
    [switch]$NoOptimize
)
...
MPLBACKEND=Agg python main_table1_paper_standard.py --strict-sr $optimizeFlag --out $outQuoted
"@

$tmpScript = Join-Path $Root ".tmp_table1_paper_wsl.sh"
...
if ($exitCode -ne 0) {
    throw "WSL paper-standard Table I run failed."
}
